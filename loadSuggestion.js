...
function newBandName() {
	...
	const innerIndex =
		generated -
		(cumFreq[outerIndex - 1] !== undefined ? cumFreq[outerIndex - 1] : 0)
	...
	setTimeout(function () {
		...
		var newWidth = Math.max(
			Math.max(suggestionWidth, copyNameWidth),
			suggestionHeaderWidth
		)
...
async function loadNameData() {
	const response = await fetch("BandNames.json").catch(function (error) {
