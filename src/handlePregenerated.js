...
let newBandName = () => {
    ...
    if (!numberRegex.test(maxInput))
        maxInput = maxStoredLength.toString();
    if (!numberRegex.test(minInput))
        minInput = minStoredLength.toString();
    ...
    if (maxSelection > maxStoredLength)
        maxSelection = maxStoredLength;
    // Fix minSelection to not exceed either min or max boundaries
    if (minSelection < minStoredLength)
        minSelection = minStoredLength;
    if (minSelection > maxStoredLength)
        minSelection = maxStoredLength;
    ...
    const possibleNames = cumFreq[maxSelection] -
    ...
    const minGen = cumFreq[minSelection - 1]
        ? cumFreq[minSelection - 1]
        : 0;
    // Generate an index minGen <= index < minGen + range
    let generated = Math.floor(Math.random() * possibleNames) + minGen;
    ...
    setTimeout(() => {
        ...
        let suggestionHeaderWidth = suggestionHeader.getBoundingClientRect().width;
        let copyNameWidth = copyName.getBoundingClientRect().width;
        let refreshNameWidth = refreshName.getBoundingClientRect().width;
        let switchModeWidth = switchMode.getBoundingClientRect().width;
        // Determine width of the longest of the three elements on the popup
        let newWidth = Math.max(Math.max(Math.max(suggestionWidth, copyNameWidth), // Longest element of the buttons or the name
